...
By default, ggvega defines a Vega-lite specification with layers. In the
Vega-Lite-world, this is a type of view composition (`facet`, `concat`,
and `repeat` being the other options for view composition) where the
charts on superimposed on top of another. While this has become second
...
are specified at the top-level of the specification. Alternatively, in a
layered chart, multiple specifications are placed into an array under
the layer property.
