...
In the R code, each example is referred-to using the `example` keyword. Above, `example ` is `"point_iris"`.
...
Given a ggspec `example` and a directory to put it, we want a function that will:
...
Given a vegaspec `example` and a directory to put it, we want a function that will:
...
Next, we copy all the examples from the source directory to the package-example directory - but only those examples that are tested as vegaspecs.
...
TDDO: Some words here on what goes into a new example, and how it becomes "official". 
